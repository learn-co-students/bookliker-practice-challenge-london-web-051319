...
const likeBook = (bookData) => {
   if (bookData.users.some(user => user.id === user1.id)) {
      console.log('You have unliked this book!');
      deleteLike(bookData);
   } else {
...
const deleteLike = (bookData) => {
   let index;
   index = bookData.users.indexOf(user => user.id === user1.id);
   bookData.users = bookData.users.slice(0, index);
